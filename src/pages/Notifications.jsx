...
export default function Notifications() {
  ...
  const { refetch } = useCustomerNotifications();
  ...
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/customer/notifications", { params: { pageSize: 50 } });
      setItems(data?.data || []);
    } finally {
      setLoading(false);
    }
  ...
  const open = async (n) => {
    if (n.status === "unread") {
      try {
        await api.post(`/customer/notifications/${n.id}/read`);
        refetch();
      } catch (_) {}
    }
  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 right-0 h-[150px] bg-gradient-to-b from-[#540101] to-[#2A0000] z-0" />
