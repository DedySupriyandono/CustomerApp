...
export default function SalesProductDetail() {
  ...
  useEffect(() => {
    if (!warehouse?.id) {
      // Belum pilih warehouse — kembali ke list supaya picker muncul.
      navigate("/sales/order-products", { replace: true });
      return;
    }
    setLoading(true);
    salesApi
      .get(`/sales/products/Details/${encodeURIComponent(uid)}`,
  ...
  const toggleSerial = (v) =>
    setSelectedSerials((prev) => (prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v]));
      ...
      <div className="relative z-10 pb-32">
        <header className="flex items-center justify-between px-5 pt-12 pb-6">
          <div className="flex items-center gap-3.5">
            ...
            <h1 className="text-white text-base font-bold">Detail Produk</h1>
          </div>
        ...
        <section className="bg-[#FBF9F9] rounded-t-[20px] -mt-2 min-h-[calc(100vh-180px)] px-5 pt-[18px]">
          ...
          <h2 className="font-bold text-[18px] text-[#1A0000] leading-tight">{data.productName}</h2>
                  ...
                  data.stockList.map((s) => {
                    ...
                    return (
                      <label key={s.id} className="flex items-center gap-3 px-4 py-3 cursor-pointer">
                        ...
                        />
                        <span className="font-mono text-[14px] text-[#1A0000]">{s.productNameValue}</span>
                      </label>
                    );
                  })
                ) : (
                  <div className="px-4 py-6 text-center text-gray-400 text-sm">Tidak ada nomor</div>
                )}
              </div>
