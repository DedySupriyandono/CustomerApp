...
// Laporan Penjualan Saya — data dari customer_stock_values (Status=Sold).
// Header per sold_ref (= sale_number "SELL-yyyymmddhhmmss-cid"). Klik header
// untuk expand & lihat detail SN.
//
// Filter: date range (from–to, default 30 hari terakhir) + search sold_ref.
// Backend endpoint: GET /customer/sell/history & /customer/sell/history/:ref
...
export default function SalesHistory() {
  ...
  // Expanded card → cached detail. Key = saleNumber.
  ...
  const toggleExpand = async (saleNumber) => {
    ...
    try {
      const r = await api.get(`/customer/sell/history/${encodeURIComponent(saleNumber)}`);
