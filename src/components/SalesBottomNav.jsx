...
export default function SalesBottomNav() {
  ...
  return (
    <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] bg-white border-t border-gray-100 flex justify-between items-end px-6 pb-6 pt-3 z-50">
      ...
      {/* Approval di-hide → ganti dgn Transaksi (order aktif).
          Transaksi kanan → ganti dgn Permintaan (order final: Selesai/Dibatalkan). */}
      ...
      {/* Center button: Scan (jualan dari stock sales). */}
      <button onClick={() => navigate("/sales/sell")} className="relative flex flex-col items-center justify-end -mt-10 mb-1">
        ...
        <span className="text-[10px] font-medium text-gray-400 mt-2">Scan</span>
      </button>
      <Item onClick={() => navigate("/sales/permintaan")} icon={<FileText className="w-6 h-6" />} label="Permintaan" active={active("/sales/permintaan")} />
